...
u3 = u0;
u4 = u0;
u5 = [u0,u0]; % leap frog 2 steg behovs
u6 = u5;  % leap frog 2 steg behovs 
%plot(x,real(u0))
...
while T < 1
   ...
   temp = u5(:,2); %nasta tids steg n+1 :u_n
   u5(:,2) = u5(:,1) - 2*dt*Central2*u5(:,2);% u5(:,1) f?rg?ende tids steg %u5(:,2):u_n+1
   u5(:,1) = temp;% det blir som en for loop
