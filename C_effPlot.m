...
C_dfem = sqrt(3)*[1.4200, 1.0800, 0.8845, 0.7684, 0.6766, 0.6120, 0.5592, 0.5142, 0.4788, 0.4501];
...
plot(degrees,C_uni,'-*','LineWidth',width);
hold on;
plot(degrees,C_gaussLobatto,'-*','LineWidth',width);
plot(degrees,C_gaussLobattoExact,'-*','LineWidth',width);
plot(degrees,C_dfem,'-*', 'LineWidth',width)
plot(2*(1:5),CFL,'-*','LineWidth',width);
hold off;
xlim([degrees(1), degrees(end)]);
legend('FEM - evenly spaced points with exact integration', 'FEM - GL points with GL quadrature', 'FEM - GL points with exact integration', 'DFEM', 'FDM - with central difference scheme', 'Location', 'northeast');
...
plot(1./C_uni,'-*','LineWidth',width);
hold on;
plot(1./C_gaussLobatto,'-*','LineWidth',width);
plot(1./C_gaussLobattoExact,'-*','LineWidth',width);
plot(degrees,1./C_dfem,'-*', 'LineWidth',width)
plot(2*(1:5),1./CFL,'-*','LineWidth',width);
hold off;
legend('FEM - evenly spaced points with exact integration', 'FEM - GL points with GL quadrature', 'FEM - GL points with exact integration', 'DFEM', 'FDM - with central difference scheme', 'Location', 'northwest');
xlabel('Degree (FEM), Order (FDM)')
